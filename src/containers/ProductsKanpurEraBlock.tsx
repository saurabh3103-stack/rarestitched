...
const ProductsKanpurEraBlock: React.FC<ProductsProps> = ({
  sectionHeading = 'Kanpur Era',
  className = 'mb-12 md:mb-14 xl:mb-16',
  variant = 'default',
  limit = 10,
}) => {
  const { t } = useTranslation();
  const kanpurEraSettings = siteSettings?.homePageBlocks?.KanpurEra;
  ...
  const {
    ...
    limit,
    tags: kanpurEraSettings?.slug,
  });

  if (!loading && isEmpty(products)) {
    return <NotFoundItem text={t('text-no-kanpur-era-products-found')} />;
  }
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between mb-5 md:mb-6">
        ...
        <Link
          href={`/collections/${kanpurEraSettings?.slug}`}
            ...
            <>
              {variant === 'default' ? (
                ...
                <Carousel
                  ...
                  prevActivateId="kanpurEraSlidePrev"
                  nextActivateId="kanpurEraSlideNext"
                >
                  {products?.map((product: Product) => (
...
export default ProductsKanpurEraBlock;
