...
const ProductsTShirtsUnder599Block: React.FC<ProductsProps> = ({
  sectionHeading = 'T-Shirts Under Rs599',
  className = 'mb-12 md:mb-14 xl:mb-16',
  variant = 'default',
  limit = 10,
}) => {
  const { t } = useTranslation();
  const tShirtsUnder599Settings = siteSettings?.homePageBlocks?.TShirtsUnder599;
  ...
  const {
    ...
    limit,
    tags: tShirtsUnder599Settings?.slug,
  });

  if (!loading && isEmpty(products)) {
    return <NotFoundItem text={t('text-no-t-shirts-under-599-products-found')} />;
  }
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between mb-5 md:mb-6">
        ...
        <Link
          href={`/collections/${tShirtsUnder599Settings?.slug}`}
          ...
          {loading && products?.length ? (
            Array.from({ length: 10 }).map((_, idx) => (
              <ProductCardGridLoader
                key={idx}
                uniqueKey={`t-shirts-under-599-${idx}`}
              />
            ))
          ) : (
            <>
...
export default ProductsTShirtsUnder599Block;
